...
`~/.agents/skills/` is the user's hand-managed skill dir: entries are **symlinks to local development repos**, not installs to clean. Examples: `using-git-agent -> ~/Developer/FradSer/git-agent/git-agent-cli/skills/using-git-agent`, `apple-events -> ~/Developer/FradSer/event/skills/apple-events`, `apple-notes -> ~/Developer/FradSer/note/skills/apple-notes`. The git-agent CLI repo ships its own `using-git-agent` skill as a short discovery stub. Agents should read the version-matched embedded reference with `git-agent skills get core` before using the CLI; the skill itself should not duplicate the full command guide or install/configuration instructions.
