...
- Settled 2026-08 (grilled tool-surface slimming): ALL lifecycle tools adopt this pattern, not just monitor/agent-teams spawn. agent-teams `teammate_spawn`/`teammate_shutdown` follow it (`[agent] started · @name · task` / `[agent] event · @name shut down`); the `[message] from/to @…` custom-message design for teammate reports is separately confirmed-good and must not be remodeled as started/event rows. Recorded in packages/agent-teams/AGENTS.md.
