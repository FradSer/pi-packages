...
user — via `ask the user in conversation` — whether to merge, before writing anything. **Merging is
...
```bash
bash scripts/arm-closeout.sh "$PR"                # explicit ask
bash scripts/arm-closeout.sh "$PR" --auto-merge   # opt-in
```
...
```bash
bash scripts/clear-closeout.sh "$PR"
```
...
- **Simple checks — judge directly.** e.g. `gh pr view "$PR" --repo "$REPO" --json state,mergedAt`
  ...
  resolved).
- **Complex or ambiguous situations — run an independent clean reasoning pass** (no authorship bias).
...
When `--auto-merge` was parsed in Phase 1, the closeout swaps the `ask the user in conversation` step for
...
hook enforces the explicit ask, then fall back to the explicit `ask the user in conversation` (four
...
2. **Ask the merge question** via `ask the user in conversation` (Phase 4 gate holds). This is the first
...
6. Merge step — depends on the opt-in:
   ...
   - **`--auto-merge` opt-in**: the gate held with zero open `escalate` items, so step 2
     was skipped; after the ceremony (steps 3–5), send a `notify the user in conversation` then run
...
7. After a successful merge: remove the linked worktree (``git worktree remove` on the `.pi/worktrees/` path`) + switch to `main` + sync `main`/`develop` + delete merged local branches + `git worktree prune` + scan stale worktrees (see "After a successful merge" above).
8. `stop re-polling` the poll script (the closeout state is already cleared).
...
- Do not merge without the user's explicit choice OR the `--auto-merge` opt-in — the merge
  requires an explicit `ask the user in conversation` choice every time, unless the user set `--auto-merge`
