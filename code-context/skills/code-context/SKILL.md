...
Use Pi built-ins: `bash`, `read`, `edit` (rarely), `write` (rarely).

| Method | Runtime |
|--------|---------|
| DeepWiki | Optional MCP (`deepwiki-code-context` in package `.mcp.json`) |
| Context7 | Optional MCP (`context7-code-context`) |
| Exa | Optional MCP (`exa-code-context`; `EXA_API_KEY` for higher limits) |
| Git clone | `bash` + `read` (always available) |
| Web search+fetch | `bash` + `curl` (or host HTTP); no built-in web search via `curl`/HTTP/HTTP fetch via `curl` tools |

If an optional MCP method is unavailable, skip it and use clone/web fallbacks. Report which methods ran.

## Token Isolation (Critical)

Prefer small intermediate summaries over dumping entire MCP/API payloads into the long-lived conversation:
...
**Tools** (MCP when configured): `read_wiki_structure`, `read_wiki_contents`, `ask_question`

**Process**:
1. `read_wiki_structure` with owner/repo (e.g. `"facebook/react"`)
2. `read_wiki_contents` for relevant topics, or `ask_question` for targeted queries
3. Extract architecture summary and key relationships

**Fallback without MCP**: Method 4 (clone) + Method 5 (web) on GitHub docs/README.
...
**Tools** (MCP when configured): `resolve-library-id`, `query-docs`

**Process**:
1. `resolve-library-id` with library name
2. When version is given (e.g. `react@18`), pin via library ID path from `versions` list
3. `query-docs` with `libraryId` and specific `query`

**Fallback without MCP**: official docs URLs via `curl`, or clone of the library repo docs.

## Method 3: Exa Code Search

Best for: real-world usage patterns and cross-web code examples.

**Tool** (MCP when configured): `get_code_context_exa`

**Process**:
1. Call with a precise query (language/framework/version/identifiers)
2. Prefer recent sources; dedupe mirrors

**Fallback without MCP**: GitHub code search URLs / known docs via `curl`, or clone representative examples.
...
- **Repo target** — `owner/repo` or git URL → DeepWiki (if MCP) or Git Clone
- **Library target** — package name / `name@version` → Context7 (if MCP) else web/docs clone
- **Natural-language target** — question/comparison → Exa (if MCP) and/or Web; Context7 if a library is named
