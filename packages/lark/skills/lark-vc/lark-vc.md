...
| 查询进行中的会议、会中事件或发送会中消息 | 本 skill 的 `+meeting-list-active` / `+meeting-events` / `+meeting-message-send`，也可由 [lark-vc-agent](../lark-vc-agent/lark-vc-agent.md) 编排 |
...
| Agent 真实入会/离会 | [lark-vc-agent](../lark-vc-agent/lark-vc-agent.md) |
...
| **进行中会议**的实时事件流（转写、聊天、共享、会中加入/离开）| `vc +meeting-events` | 本 skill / [`lark-vc-agent`](../lark-vc-agent/lark-vc-agent.md) |
