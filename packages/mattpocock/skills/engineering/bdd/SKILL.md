---
name: bdd
description: "Behavior-driven development: Discovery → Formulation → Automation. Use when the user wants to build features or fix bugs via Gherkin scenarios and the BDD lifecycle, or mentions BDD/Gherkin/Executable Specifications. Delegates test implementation to /skill:tdd."
---

# Behavior-Driven Development (BDD)

BDD is not just about tools; it's a methodology for shared understanding and high-quality implementation. This skill governs the **Discovery → Formulation → Automation** lifecycle: Gherkin scenarios define the behavior, and the Automation phase (red-green loop) is delegated to `/skill:tdd` (BDD-driven).
...
*   **Automation:** Implement using the red-green-refactor cycle via `/skill:tdd`, which governs test quality, seams, mocking, and anti-patterns.
...
The Automation phase (red-green loop) is governed by the `/skill:tdd` skill (BDD-driven). When the Gherkin scenario is defined and the seam is agreed, invoke `/skill:tdd` for:
...
Tests verify behavior through public interfaces, not implementation details. See `/skill:tdd` for the full guidance, examples, and anti-patterns.
...
See `/skill:tdd` (BDD-driven) for the full red-green loop rules, anti-patterns, and test quality guidance. The key points:
...
- `/skill:tdd` - BDD-driven test implementation: seams, mocking, test quality, red-green loop
