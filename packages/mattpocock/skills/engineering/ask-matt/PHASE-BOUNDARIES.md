...
| **`/skill:handoff`** | Write a portable Markdown document; it does not create, fork, or seed a session. |
...
That list is the whole clause. What `/skill:handoff` buys is **portability** — a document that travels. It only writes the document; it does not create, fork, or seed a session. If nothing is travelling, you don't need it.
