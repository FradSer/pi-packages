...
5. **Research the frontier.** For each `research` ticket you just created, use `/skill:research`. Delegate independent tickets to teammates only when the teammate facility is available; otherwise resolve them sequentially. Capture findings on a throwaway `research/<name>` branch with a context pointer from the ticket.
