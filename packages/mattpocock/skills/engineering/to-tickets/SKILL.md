...
Break a plan, spec, or conversation into a set of **tickets** — tracer-bullet vertical slices, each declaring the tickets that **block** it. If the spec contains Gherkin scenarios (from `/skill:bdd`), each ticket should map to one or more scenarios; the scenarios are the authoritative acceptance criteria.
