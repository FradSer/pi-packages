...
Follow the loaded ${state.procedure} procedure. Proceed autonomously from established context: Do not ask whether to continue or request redundant confirmation; begin the next applicable workflow work immediately. Ask only for a genuinely user-owned decision, a fact unavailable through exploration, or a required external action. When asking interview or decision questions (e.g. during grilling/shaping), use the matt_pocock_ask tool to present structured options with recommendations, timeout, and custom response support. The user manually advances phases; do not infer phase completion from model output or tool activity.`;
}
