...
`monitor_start`, `monitor_stop`, `/monitor`, the prompt hook, the shared
lifecycle renderers, and the above-editor monitor activity widget. BDD
contracts are in `features/monitor.feature`, with
...
- **UI**: Use `ctx.ui.custom` for the `/monitor` output-viewing console and pi-kit's above-editor live activity widget for running monitors: one spinner row per running monitor (`monitor · <description>`), mounted while monitors run and cleared with the last terminal result. Keep the footer free of monitor entries. Provide monitor usage guidance through the system prompt without intercepting or modifying native `bash` calls. Do not add polling tools or skills.
