Feature: Control only live Pi sessions through private sockets
  ...
  Scenario: Deliver through the real SDK without provider credentials
    ...
    When the CLI sends a literal prompt to its live socket
  ...
  Scenario: Use stdin rather than shell arguments
    Given the JSON-lines CLI is running
    When a list request arrives on stdin
    Then exactly one versioned JSON response is written on stdout
  Scenario: Reject an oversized unfinished stdin frame exactly once
    Given the JSON-lines CLI is running
    When more than 65536 bytes arrive without a newline
    Then exactly one frame_too_large response is written
    And no incomplete_frame response follows
