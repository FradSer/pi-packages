// Modified for @fradser/pi-impeccable: Pi-adapted live boot: agent-owned context preparation replaces unavailable upstream CLI commands.
...
Options:
  ...
  if (missingContext.length > 0) {
    console.log(JSON.stringify({
      ...
      nextStep: missingContext.includes('PRODUCT.md')
        ? 'Prepare PRODUCT.md with the user (product, audience, brand personality), then rerun this boot.'
        : 'Prepare DESIGN.md with the user (visual system: color, type, layout), then rerun this boot.',
