#!/usr/bin/env node
// Modified for @fradser/pi-impeccable: Pi-adapted copy-edit runner: runPi provider ahead of upstream host CLIs.
...
export async function runCopyEditBatchAgent(batch, opts = {}) {
  ...
  if (provider === 'pi') {
    await runPi(prompt, { cwd, env, resultPath, logPath, timeoutMs: opts.timeoutMs });
...
function runPi(prompt, { cwd, env, resultPath, logPath, timeoutMs = DEFAULT_TIMEOUT_MS }) {
  const args = [
    '--print',
    '--no-session',
  ...
  // The final assistant text on stdout IS the result payload; mirror it to
  // resultPath the way the claude runner mirrors its JSON envelope.
  return runAgentProcess('pi', args, prompt, { cwd, env, logPath, timeoutMs, mirrorOutputPath: resultPath });
}
...
export function describeNoProviderError({
  ...
  if (exists('pi')) {
    lines.push('  • Pi CLI: installed. If Apply still fails, check provider authentication with `pi auth check` or a trivial `pi --print` prompt.');
  } else {
    lines.push('  • Pi CLI: not installed.');
  }
  if (exists('claude')) {
...
function computeCommandAuthed(command) {
  ...
  if (command === 'codex' || command === 'pi') return true;
