    ...
    evtSource.onmessage = (e) => {
      ...
      switch (msg.type) {
        case 'connected':
          ...
          if (!hasProjectContext) showToast('No PRODUCT.md found. Variants will be brand-agnostic. Ask your agent in chat to prepare PRODUCT.md with you, then re-open this panel.', 7000);
...
void main() {
  ...
  function renderDesignBody() {
    ...
    if (designState.present === false) {
      ...
      empty.innerHTML = '<strong>No DESIGN.md yet</strong>Ask your agent in chat to prepare DESIGN.md with you, then re-open this panel.';
  ...
  function renderStaleHint() {
    ...
    box.innerHTML = `
      ...
      <span class="stale-text"><strong>DESIGN.md is newer than .impeccable/design.json.</strong> Ask your agent in chat to refresh the sidecar.</span>
    `;
    return box;
  }
  ...
  function renderParsedMdCta() {
    ...
    box.innerHTML = `<strong>Basic view</strong>This panel reads the tokens in your <code>DESIGN.md</code> frontmatter. A <code>.impeccable/design.json</code> sidecar with your project's actual component snippets (button, input, nav) and tonal ramps is rendered live below the tokens when present.`;
    return box;
  }
  ...
  function designEmptyMessage() {
    if (designState.hasMd && !designState.hasSidecar) {
      return 'DESIGN.md found, no structured tokens to display. Ask your agent in chat to generate the .impeccable/design.json sidecar.';
    }
