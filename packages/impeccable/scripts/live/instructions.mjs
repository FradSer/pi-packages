// Modified for @fradser/pi-impeccable: Pi-adapted agent instructions: actions map to shipped capabilities via impeccable_load.
...
const PLAN_POINTER = 'Plan per the live procedure, planning section: extract the identity lock, pick default vs departure mode, commit each variant to a DIFFERENT primary axis, squint-test the trio. Size parameter knobs per the parameters section budgets.';
...
function generateInstructions(event, scriptsPath) {
  ...
  steps.push(event.action && event.action !== 'impeccable'
    ? `Action is "${event.action}": load that action's guidance first via impeccable_load {"capability":"${event.action}"}; its MUST params are non-negotiable. ${PLAN_POINTER}`
    : `Freeform action: load the taste guidance you need via impeccable_load {"capability":"live","reference":"<id>"} (principles, components); no per-action reference. ${PLAN_POINTER}`);
...
function insertScaffoldInstructions(event, scriptsPath) {
  const scaffold = event.scaffold;
  const base = `Insert mode: net-new content sized around ${event.placeholder?.width || '?'}x${event.placeholder?.height || '?'} at the chosen anchor; load the components taste guidance via impeccable_load {"capability":"live","reference":"components"} before writing net-new markup.`;
