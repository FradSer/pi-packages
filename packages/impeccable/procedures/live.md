...
Pi adaptation of Impeccable `skill/reference/live.md` at `63b04e2530f5c7b41ea83c133daab24f34912456`, Apache-2.0. Modified for Pi: helper paths resolve through this package, one Pi harness policy replaces per-host policies, actions map to shipped capabilities via `impeccable_load`, and unavailable upstream CLI commands are replaced with agent-owned preparation. Apache-2.0 attribution is in the package licenses and notices.
...
Pi harness policy:
- Run the poll with the session's **generic background monitor** when available: no short timeout, one terminal result, and end the turn after starting it — the monitor's completion wakes the session.
- Without a background monitor, run a **one-shot** poll per turn with a bounded `--timeout=MS`, handle, `--reply`, restart. Do **not** use `--stream` unless incremental stdout is measured reliable in this session.
- Never leave live mode without an active poll: ending a turn with the user waiting at Go and no way to be woken is a dead session. A poll you started must be serviced — through the monitor's terminal result, or by staying in the turn until the poll returns.

Delivery policy: atomic single-edit delivery everywhere; do not switch to progressive publishing unless the poll loop is known not to block on the extra calls.
...
```
LOOP:
  node "{{PKG_DIR}}/scripts/live-poll.mjs   # default long timeout; no --timeout=
...
If output is `{ ok: false, error: "config_missing" | "config_invalid", path }`, this project needs one-time configuration: load the disclosed [live setup](live-setup.md) guidance with `impeccable_load {"capability":"live","reference":"live-setup"}` and follow it. If the output carries a non-null `configDrift`, tell the user once which HTML files are uncovered and suggest adding them or switching `files` to a glob; never auto-edit the config.
...
```bash
node "{{PKG_DIR}}/scripts/live-status.mjs      # helper state, active sessions, queued events; works with the helper down
node "{{PKG_DIR}}/scripts/live-resume.mjs --id SESSION_ID   # active snapshot, pending event, next safe action
node "{{PKG_DIR}}/scripts/live-complete.mjs --id SESSION_ID # canonical manual final acknowledgement after verified cleanup
```
...
```bash
node "{{PKG_DIR}}/scripts/live-wrap.mjs --id EVENT_ID --count EVENT_COUNT --element-id "ELEMENT_ID" --classes "class1,class2" --tag "div" --text "TEXT_SNIPPET"
```
...
`event.action` is `impeccable` (freeform): load the taste guidance you need through `impeccable_load {"capability":"live","reference":"<id>"}` (principles, components); decide the visitor mode from the surface; do not load a per-action reference. Freeform is not a pass to skip parameters: follow the budget and freeform bias in section 7. Any shipped action (`polish`, `typeset`, `colorize`, `layout`, `animate`): load that capability's guidance with `impeccable_load {"capability":"<action>"}` before planning; its MUST params layer on top of the section 7 budget. The browser picker only offers shipped actions.
...
- Freeform with a named direction: honor the direction through each variant's own primary axis.
...
```bash
node "{{PKG_DIR}}/scripts/live-poll.mjs --reply EVENT_ID done --file RELATIVE_PATH
```
...
When the session can spawn teammates or sub-agents, delegate source edits to one. Pass cwd, scripts path, event id, page URL, chunk/deadline, `batch`, `evidencePath`, and the canonical JSON result schema. The delegate must not poll or reply. If unavailable, apply inline with the same contract.
...
```bash
node "{{PKG_DIR}}/scripts/live-server.mjs stop
```
...
Only when the boot reports `config_missing` / `config_invalid`, or `configDrift` needs explaining, or the config lacks `cspChecked`: load the disclosed [live setup](live-setup.md) guidance. It owns the config schema, the per-framework `files` table, injection adapters, drift healing, and the CSP detection and consent flow.
