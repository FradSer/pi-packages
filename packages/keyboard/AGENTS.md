...
`state-machine.ts` (lifecycle state and
...
Keep Pi lifecycle wiring and `/keyboard` handling in `src/index.ts`; keep
external CLI execution in `driver.ts`.
...
definitions, zones, lifecycle transitions, session cleanup,
