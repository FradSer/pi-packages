...
- Message routing results describe only the synchronous transition: `steered` means the active control stream accepted the write; `queued` means the harness still owns delivery into a recipient turn. Inbox/outbox writes are `queued`, never `delivered`, and neither label claims the recipient read or processed the message.
