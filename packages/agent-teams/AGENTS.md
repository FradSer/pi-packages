...
- The startup moment owns exactly one row in `renderResult`: `[agent] @<name> started · <assignment>`, built with `formatToolLifecycleTitle(startedToolLifecycle("agent", …))` (the teammate name is part of the subject, with the `started` state following it). Only the `[agent]` prefix is accent-colored; the teammate, state, and assignment use the default text color. The native `Text` component wraps at the available width; the row has no lifecycle background band, expansion hint, or transcript details.
...
- **Progressive Tool Disclosure**:
  - *Leader tools (`refreshLeaderToolDisclosure`)*: `teammate_spawn` and `task_create` are always available; `teammate_shutdown` and `send_message` are revealed only when living teammates exist; `task_list` is revealed when board tasks exist.
  - *Worker tools (`createWorkerToolDisclosure`)*: `task_list` and `task_claim` are revealed only when a board notice arrives; `task_submit` is revealed only after the worker successfully claims a task and removed upon submission.
