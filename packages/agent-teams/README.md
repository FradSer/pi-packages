...
- Leader steering or direct assignment: `to: "<teammate-name>"`; assign work once, then send only new evidence, changed constraints, or decisions relevant to that assignment. Include the new fact and its impact. Workers complete and report autonomously; progress requests and repeated instructions add no value. Without new information, do independent work or yield. The synchronous routing result is `steered` only when the active control stream accepts the message, otherwise `queued` while the harness owns delivery on the next wake-up.
...
- **Event-driven coordination**: reports are handed to Pi immediately, preserving their event IDs, original bodies, and authored timestamps. No report waits for a previous leader response. Synchronous handoff failures notify the user; Pi reports asynchronous runtime failures through its error channel. The console mailbox retains the report, without speculative timeout-based resends.
- **Yield while teammates work**: continue independent work while teammates run; if none remains, end the turn. Send a worker newly discovered information only when it changes the assignment. Reports arrive automatically; do not extend the turn with `sleep`, polling, `task_list`, status requests, or repeated guidance.
