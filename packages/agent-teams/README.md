...
- **Yield while teammates work**: continue independent work while teammates run; if none remains, end the turn. Reports resume the session automatically, so do not extend the turn with `sleep`, polling, `task_list`, status requests, or unsolicited steers.
