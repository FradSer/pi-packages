...
- No `prompt` means inspection only. Presence includes the actual Work Sessions, work IDs, execution states, and precise message routes.
...
- Leader steering or direct assignment: `to: "<teammate-name>"`; assign work once, then send only new evidence, changed constraints, or decisions relevant to that assignment. Include the new fact and its impact. Workers complete and report autonomously; progress requests and repeated instructions add no value. Without new information, do independent work or yield. The synchronous routing result is `steered` when the control stream accepts a native prompt with steering behavior. Pi then steers active execution or starts idle execution; otherwise the result is `queued` while the harness owns delivery on the next wake-up. RPC rejection is reported separately as a harness diagnostic.
...
- **Yield while teammates work**: continue independent work while teammates run; if none remains, end the turn. Yielding is not task completion: before claiming completion, account for every current assignment with its terminal report and acceptance evidence, or explicitly hand off the unfinished scope. Send a worker newly discovered information only when it changes the assignment. Reports arrive automatically; do not extend the turn with `sleep`, polling, `task_list`, status requests, or repeated guidance.
