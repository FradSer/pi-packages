Feature: Native code-context tool behavior
  To expose only usable Pi-native interfaces and truthful retrieval failures
  As a user of @fradser/code-context
  I want the documented skill surface and tool results to match the Pi runtime

  Scenario: The README advertises the executable skill rather than nonexistent commands or agents
    Given the code-context package is installed in Pi
    When I read its README
    Then it documents /skill:get-context as the executable workflow
    And it may describe context-researcher only as an optional manual prompt brief
    But it does not advertise /get-context or @context-researcher invocation

  Scenario: An in-flight provider lookup is cancelled by Pi
    Given a code-context lookup is waiting for an HTTP response
  ...
  Scenario: A retrieval provider is unreachable
    Given a code-context HTTP request fails before receiving a response
  ...
  Scenario: A provider request exceeds its timeout
    Given a code-context HTTP request has no Pi execution signal
