...
- Aim for 200-400 words for a repository-level guide; keep nested guides shorter and focused on their directory.
- Cover Project Structure & Module Organization, Build/Test/Development Commands, Coding Style & Naming Conventions, Testing Guidelines, and Commit & Pull Request Guidelines when relevant to that scope. Add Security, Configuration, Architecture, or Agent-Specific Instructions only when supported by repository evidence.
- Include concrete commands and paths where helpful.
- Keep a professional, instructional tone. Do not claim tools, workflows, or policies that you did not verify.

After editing, briefly report which instruction files were created or updated and the independent directory scope each file covers.
`.trim();
