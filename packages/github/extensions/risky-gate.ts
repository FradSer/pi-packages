/**
 * Option-based gate for irreversible GitHub operations.
 *
 ...
 * attempts a risky command (merging a PR, force-removing a worktree), the
 * hook does NOT ask a yes/no on that pre-built command. Instead it asks the
 * ACTIVE MODEL to generate the OPTIONS the user might choose (the proposed
 * action, a safer alternative, cancel) plus the exact command for each
 * option, then pops pi's native select dialog. The user picks an option and
 * the command for that choice runs — the model generates the commands, the
...
const RULES: GateRule[] = [
	{
		...
		risk: "Merging is hard to reverse and outward-facing.",
	},
	{
