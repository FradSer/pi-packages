...
3. **Mandatory handoff to `/skill:review-pr`** — the review → fix → commit+push → wait-for-review loop, until CI is green and every comment is triaged, then asks the user whether to merge via the **`gh_ask_merge`** native dialog (merge commit/squash/rebase/don't; in non-TUI it degrades to a conversation ask) **before** the closeout ceremony — the summary comment and body rewrite run only on a merge choice. This handoff is default-on; skipped only on explicit `--no-monitor` or user opt-out.
4. **Post-merge branch + worktree hygiene** — delegated onward to `/skill:review-pr` (Phase 5 closeout), which deletes the remote + local head branches (when stack-safe and in the main worktree), removes the linked worktree (``git worktree remove` on the `.pi/worktrees/` path`), switches to `main`, fast-forwards local `main`/`develop` with origin, drops all other already-merged locals, runs `git worktree prune`, and scans for stale worktree directories. See `references/closeout.md`.
...
- Pass `--auto-merge` through **only** on an explicit user opt-in (never infer it). create-pr forwards it to `/skill:review-pr` as ``/skill:review-pr <PR#> --auto-merge``; review-pr then skips the `gh_ask_merge` step — the closeout ceremony (summary comment + body rewrite) still runs first — and runs `gh pr merge --merge` once CI is green and every non-escalate comment is triaged. `escalate` items suspend the opt-in and fall back to the `gh_ask_merge` dialog — see `review-pr/references/closeout.md` (Auto-merge branch).
...
- Every PR enters the `/skill:review-pr` loop after creation — review, fix what is verified, commit+push, wait for the next review round — until CI is green and every comment is triaged, then the user is asked whether to merge before the closeout ceremony.
