...
- **On PR branch**: Ask "Must this be fixed before merge?" via the **`gh_confirm`** tool
  (title: "Fix before merge?", message: summarize the issue and the current PR branch). The
  tool renders a native yes/no dialog and blocks until the user answers; in non-TUI mode it
  reports `confirmed: null` — then ask in the conversation instead and wait.
