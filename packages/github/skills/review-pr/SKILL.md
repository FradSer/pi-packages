...
| AskUserQuestion | Call the **`gh_ask_merge`** tool (native dialog; blocks until answered) or `gh_confirm` for yes/no; in non-TUI mode both degrade to asking in the conversation |
...
1. Call the **`gh_ask_merge`** tool with `pr` (and `escalateCount` when unresolved escalate comments remain) — it renders the 4-option native dialog and blocks until the user answers. The answer cannot be hallucinated: the tool only returns after a real dialog interaction.
2. Do not post summary/body before the merge decision — the ceremony runs only on a merge choice. If the tool reported `mode: "conversation"` (non-TUI), ask in the conversation instead and wait for the reply.
...
8. Merge only after explicit user choice; never `gh pr merge --auto`. **`--auto-merge`**: when set in Phase 1, skip the ask but still run ceremony first, then `gh pr merge --merge` once green and non-escalate comments triaged. Open `escalate` suspends auto-merge — fall back to the `gh_ask_merge` dialog.
