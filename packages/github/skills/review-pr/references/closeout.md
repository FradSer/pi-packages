...
the last action before `stop re-polling`.

## Merge decision — ask first

The Phase 4 gate holds: every `[ci]` check terminal + passing, every comment reflected on
(resolved ones hidden + threads resolved; only `escalate` items remain visible). Call the
**`gh_ask_merge`** tool (from this package's `extensions/interactive.ts`) with the PR number
before writing anything. **Merging is hard to reverse and outward-facing**, so it requires an
explicit user choice every time; never auto-merge, and never merge past open `escalate`
comments without surfacing them.

The tool renders a native dialog with four mutually exclusive options and **blocks until the
user answers — the answer cannot be hallucinated**, because the tool only returns after a
real dialog interaction. In non-interactive runs (`pi -p` / headless) the tool reports
`mode: "conversation"`; then ask in the conversation instead, with the same four options,
and wait for the reply. Pass `escalateCount` when unresolved `escalate` comments remain so
the question carries the count and the user merges with eyes open. The user may still choose
to merge — that is their call, not the skill's.
...
- **On a merge choice**: proceed to the ceremony below, then run the matching `gh pr merge`
  ...
  If the closeout runs in a linked worktree (`/skill:resolve-issues`), omit `--delete-branch`
  — delete the remote head separately if stack-safe, leave the local branch for `git worktree remove`.
  Never `--auto`.
- **On "Don't merge"**: skip the ceremony and post-merge hygiene entirely; fall through to
  `stop re-polling`. The ceremony is idempotent — if the user later wants the record, re-running
...
When `--auto-merge` was parsed in Phase 1, the closeout swaps the `gh_ask_merge` step for
...
Fall back to the `gh_ask_merge` dialog (four options, merge listed first as Recommended),
passing `escalateCount` so the question carries the count. Do not merge past escalate items
just because the flag was set — escalate means "needs human judgment", and auto-merging past
it is exactly the over-reach the explicit-choice rule exists to prevent. The user may still
pick merge from the question; that is their call.
...
2. Send a `notify the user in conversation` that the PR is about to auto-merge — merge is hard to reverse and
...
3. Run `gh pr merge "$PR" --repo "$REPO" --merge --delete-branch` (safe only in the main
   ...
   branch for `git worktree remove`). Never `--auto`.
...
different flags, do not force-push, and do not retry auto-merge; the user decides next. If
...
back to the `gh_ask_merge` dialog.

**On a successful auto-merge**, proceed to "After a successful merge" hygiene exactly as the
explicit-choice path would. `stop re-polling` the poll script.
...
1. **Remove the linked worktree first, before any switch.** Detect whether the closeout is
   running in a linked worktree (`/skill:resolve-issues`):
   ```bash
   [ "$(git rev-parse --git-dir)" != "$(git rev-parse --git-common-dir)" ] && echo "linked worktree"
   ```
   If linked, ``git worktree remove` on the `.pi/worktrees/` path` — it deletes the worktree and its local branch
   and returns the session to the main worktree. If it refuses (uncommitted changes), call the
   **`gh_confirm`** tool before `discard_changes: true`. Then delete the remote head separately if
...
5. Scan `.pi/worktrees/` for stale worktree directories whose branch is already merged
...
1. **Ask the merge question** via the **`gh_ask_merge`** tool (Phase 4 gate holds; in non-TUI
   it degrades to a conversation ask). This is the first closeout step — the ceremony below
   runs only on a merge choice.
...
5. Merge step — depends on the opt-in:
   ...
   - **`--auto-merge` opt-in**: the gate held with zero open `escalate` items, so step 1
     was skipped; after the ceremony (steps 2–4), send a `notify the user in conversation` then run
     ...
     auto-merge and fall back to the `gh_ask_merge` dialog.
6. After a successful merge: remove the linked worktree (``git worktree remove` on the `.pi/worktrees/` path`) + switch to `main` + sync `main`/`develop` + delete merged local branches + `git worktree prune` + scan stale worktrees (see "After a successful merge" above).
7. `stop re-polling` the poll script.
...
comment or CI re-ran red), do not auto-merge — re-check and fall back to the `gh_ask_merge`
dialog if escalate items now exist.
...
- Do not end the turn without resolving the merge decision — call `gh_ask_merge` and act on
  its answer (any choice, including "Don't merge") before stopping the poll.
...
- Do not merge without the user's explicit choice OR the `--auto-merge` opt-in — the merge
  requires an explicit `gh_ask_merge` choice every time, unless the user set `--auto-merge`
