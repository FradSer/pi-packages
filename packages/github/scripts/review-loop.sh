#!/usr/bin/env bash
# review-loop.sh — persistent CI + PR-comment watch for the review-pr workflow
# (invoked from /github review-pr).
#
# Emits one tagged stdout line per new event. Run via Pi `bash` and re-enter
# across turns as needed (no Monitor tool in Pi).
#
# Output format:
...
#
# Env vars (PR / REPO / INTERVAL) take precedence over flags so the workflow can
# pass them as env vars to this script.
...
# comments posted before the review started surface on poll 1. GitHub's ?since=
