...
`Closes`/`Fixes`/`Resolves #N` in a PR body or commit message tells GitHub to close the linked issue when the PR merges. This is the single authoritative reference for all github plugin workflows.
...
- **`create-issues procedure`**: warn at issue-creation time when the issue is PR-scoped and the eventual PR base may be non-default.
- **`create-pr procedure`**: warn at PR-creation time (Phase 3, target base branch validation) when the PR targets a non-default branch.
- **`resolve-issues procedure`**: surfaced via the `create-pr procedure` handoff — do not re-warn here; create-pr owns the warning at PR-creation time.
