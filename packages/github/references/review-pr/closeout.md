...
user directly in the conversation** how to merge the PR before writing anything. **Merging
is hard to reverse and outward-facing**, so it requires an explicit user choice every time;
never auto-merge, and never merge past open `escalate` comments without surfacing them.

Ask one question with four mutually exclusive options and **wait for the user's explicit
reply — never merge without it** (the answer cannot be hallucinated because you act only
on what they actually typed). Surface the `escalate` count when unresolved `escalate`
comments remain so the user merges with eyes open. The user may still choose to merge —
that is their call, not the agent's.
...
- **On a merge choice**: proceed to the ceremony below, then run the matching `gh pr merge`
  ...
  If the closeout runs in a linked worktree (`resolve-issues procedure`), omit `--delete-branch`
...
When `--auto-merge` was parsed in Phase 1, the closeout swaps the merge question for
...
Fall back to asking the user in the conversation (four options, merge listed first as
Recommended), surfacing the escalate count so the question carries it. Do not merge past escalate items
...
back to asking the user in the conversation.
...
1. **Remove the linked worktree first, before any switch.** Detect whether the closeout is
   running in a linked worktree (`resolve-issues procedure`):
   ```bash
   ...
   and returns the session to the main worktree. If it refuses (uncommitted changes), ask the
   user in the conversation before `discard_changes: true`. Then delete the remote head separately if
...
1. **Ask the merge question** directly in the conversation (Phase 4 gate holds) and wait
   for the reply. This is the first closeout step — the ceremony below runs only on a merge
   choice.
...
5. Merge step — depends on the opt-in:
   ...
   - **`--auto-merge` opt-in**: the gate held with zero open `escalate` items, so step 1
     ...
     auto-merge and fall back to asking the user in the conversation.
...
comment or CI re-ran red), do not auto-merge — re-check and fall back to asking the user
in the conversation if escalate items now exist.
...
- Do not end the turn without resolving the merge decision — ask the user and act on
  their answer (any choice, including "Don't merge") before stopping the poll.
