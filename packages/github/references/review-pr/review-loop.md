# Review Loop: Poll, Triage, Auto-Fix

Monitoring uses **`scripts/review-loop.sh` (bundled in the `@fradser/github` package) via Pi `bash`** — a poll that emits
...
## One poll for CI and Comments

Run one poll script (re-enter across turns; PR reviews arrive on no fixed schedule).
...
The script is bundled with the `@fradser/github` package and lives at `scripts/review-loop.sh`
(executable, `#!/usr/bin/env bash`). Locate it (installed npm/git copy, or a `settings.json`
relative-path dev checkout), then run:

```bash
# Resolve the script path, then run
resolve_review_loop() {
  local s=$(find ~/.pi/agent .pi -path "*/github/scripts/review-loop.sh" 2>/dev/null | head -1)
  [ -n "$s" ] && echo "$s" && return
...
for p in s.get('packages', []):
    if 'github' in p:
        c = os.path.normpath(os.path.join(base, p, 'scripts', 'review-loop.sh'))
        if os.path.isfile(c): print(c); break
" 2>/dev/null)
  fi
  [ -n "$s" ] && echo "$s" && return
  echo "scripts/review-loop.sh"
}
SCRIPT=$(resolve_review_loop)
...
- `since` is seeded to the PR's **creation time** (not launch time), so comments posted
  before the review started are surfaced on poll 1. It advances to `now` after each poll.
...
Run via the poll script tool with `persistent: true` and a specific `description`
(e.g. `"CI + new comments on PR #<n> (5m poll)"`). Stop it with stop re-polling when done — never
...
The review-pr procedure's Phase 1 baseline review runs in an independent agent — the main context
...
Each emitted line is a notification. Re-run the poll after you act, so apply
...
3. Send notify the user in conversation for each `escalate` verdict with comment body + author + file context
...
Apply the validated fixes, commit and push them via the commit-and-push procedure, then acknowledge each. **The reply endpoint depends on comment type** — the `/pulls/$PR/comments/<id>/replies` endpoint ONLY accepts inline review-comment ids; using it for an issue-level comment or a review summary hits the wrong resource and fails:
...
### `[comment]` ambiguous — notify the user in conversation and report
...
For these: send a **notify the user in conversation** (the user may have stepped away), report the comment
body + author + file context, and let the user decide. Do NOT reply to or guess at these.

## Lifecycle

- Re-enter the poll across turns; you keep working and react as lines arrive.
- A pushed fix triggers a fresh CI run that the same poll script re-emits — no need to change the script — re-run it.
- A temporarily empty comment queue is NOT a stop signal — other agents may post more
  comments later. Keep the watch running.
- Stop with **stop re-polling** when EITHER holds:
  ...
  - **Hard cap (overrides the above)**: the ~2-hour max wall-clock is reached, OR the
    ...
    or comments remain. The cap exists so a stuck PR (red CI the review correctly won't
