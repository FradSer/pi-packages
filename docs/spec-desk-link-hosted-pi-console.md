...
3. **Handshake.** The control connection carries the protocol version, the reporting token, and a Console identity: the driving Pi session's identity plus its machine name. Two Pi sessions on one machine are therefore distinguishable Consoles. The desk challenges with a cryptographically random, connection-scoped one-time nonce; the Console proves possession of the Control Credential with HMAC-SHA256 over an unambiguous transcript containing a fixed domain, the protocol version, nonce, machine name, and Console session identity. Every control record is refused without that proof. The credential itself never travels, so a passive capture yields no reusable execution token. A missing or refused credential degrades the connection to report-only and never affects reporting. The challenge expires with the authentication timeout and cannot be reused on another connection.
...
5. **Control records and correlation.** Console to desk: list, launch, attach, prompt, cancel, end, and history. Desk to Console: state, event, acknowledgement, and error. There is no detach record: disconnecting, or attaching elsewhere, is what detaching means. A request identity correlates one wire exchange; a caller-generated mutation identity makes launch, prompt, cancel, and end reconcilable without blind automatic retry; Hosted Pi identity, attachment identity, and Hosted Pi Position remain separate concepts. Reusing a mutation identity with the same canonical payload returns the recorded outcome, while a different payload is refused. A cancel may name the turn it expects, so a delayed duplicate cannot cancel a later turn. Control records reuse the existing frame, request, and reply bounds, and event content reuses the existing Session Event bounds.
6. **One coordinate for live events and history.** A Hosted Pi event batch's position is the place of one corresponding complete entry in that session's own Pi session log, which the host already writes durably. One log entry may map to several Session Events—for example thinking, a tool call, and assistant text—so those events travel and are applied as one batch sharing that position; the Console advances its applied position only after applying the whole batch. Position zero means before the first entry. Positions therefore survive a host restart without a separate counter, a reset rule, or a persisted sequence store, and history and live events are addressed in the same space. This holds because a Hosted Pi is driven linearly; if branching were ever exposed, the coordinate would need revisiting.
7. **No replay window.** Attach atomically installs the live subscription and captures a boundary, so an entry cannot land between boundary sampling and subscription. On a repeated Attach, a Console states the position it last applied; the host emits history in the exclusive/inclusive interval `(last applied, boundary]`, states that it is caught up, and then emits live events strictly after the boundary. On a first Attach, the Console learns the current boundary and consumes only future live events; an explicit history read is what shows earlier work. Nothing is replayed from memory, so the desk keeps no per-session replay buffer, needs no window sizing or expiry rule, and the silent-gap failure has no way to occur. This follows the closest precedent: the comparable coding-agent server streams live events without a resume cursor and serves catch-up from an explicit position query.
...
9. **Lifecycle and turn outcome are separate.** A Hosted Pi lifecycle is `launching`, `live`, `ended`, or `interrupted`. While live, its activity is `working` or `idle`; the wire may continue projecting those two activities as `running` and `settled` for the existing Pi Sessions vocabulary. A turn outcome is separately `finished`, `failed`, `cancelled`, or `interrupted`. A finished, failed, or cancelled turn normally leaves the Hosted Pi live and idle; ending the Hosted Pi changes its lifecycle and disposes the SDK session; host recovery can mark the lifecycle interrupted when the live SDK session cannot be reconstructed. Steering uses the SDK's own queue semantics: a prompt carries a streaming behavior that is required while a turn streams and ignored when idle. A mid-turn instruction therefore steers the running turn instead of starting a second one. Cancelling aborts the running turn and returns the same Hosted Pi to live-idle without disposing it. Ending disposes the session and releases its slot while the receipt stays readable. The surface must never infer lifecycle from a turn outcome or coerce an unknown state to working.
...
16. **Failure answers name a reason.** A desk service restart must not end live sessions; a Pi host that is down or a stale host socket must produce an explicit bounded-time failure rather than a wait; a mutation whose outcome never arrived is reconciled by its mutation identity and never retried automatically; and a project that is deleted or a development root that changes while a session is alive has defined behavior. Slow attached consumers are disconnected instead of accumulating an unbounded output queue, because they can Attach again from their last applied position.
...
- **One seam: the Desk Link control contract.** The Console and the service meet at exactly one public seam, so the contract is verified there and nowhere else. A versioned corpus of canonical records and HMAC vectors is exercised by both repositories, and a loopback test verifies the two adapters together; neither repository maintains an independent interpretation.
...
- **What makes a good test here.** Tests assert observable behavior at those seams: the records on the wire, the state the desk reports, the events the Console applies, the position it resumes from, and the bounded content that enters context. They must not assert internal helpers and must not need a model or the desk device. Lifecycle and turn outcome are asserted independently: cancelling a turn must leave a live, idle, attachable Hosted Pi, while ending changes lifecycle and releases the slot.
...
- **Two contract changes outside this document are recorded, not absorbed.** Two changes touch what this specification reuses, and neither is part of the contract this document relies on. First, the Session Event bounds were raised to 300 events per session, 1,048,576 bytes, and a per-kind body limit instead of single-line summaries for non-result events; that change has landed on the reporting package and on the desk, and the package's tests assert the raised contract. This specification deliberately reuses "the existing Session Event bounds" without restating numbers; the numbers themselves appear in the reporting package's README and in the desk's Desk Link document, and both state the raised contract. The desk's Desk Link Service enforces its own copy of those numbers, because a reporting machine is never trusted to have bounded its own data, so the two sides must be raised together: a reporter sending larger numbers alone would be trimmed on the desk. Second, the desk's own session vocabulary was reworked (its session filter reads Live, Working, Idle, Exited), which is the vocabulary the control-provenance and state work extends; an implementer still re-reads that vocabulary rather than trusting any enumeration written here.
