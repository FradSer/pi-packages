...
2. Manually publish the first version: `cd packages/<name> && npm publish --access public` (browser OTP auth pops).
...
4. Re-trigger `gh workflow run release.yml --repo FradSer/pi-packages` → CI publishes the bumped version via OIDC with provenance.
