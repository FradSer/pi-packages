...
**Published packages (verified 2026-08-15):** `@fradser/pi-memory` (0.2.1), `@fradser/pi-btw` (0.1.1), `@fradser/pi-monitor` (1.0.0), `@fradser/pi-utils` (0.3.2), `@fradser/pi-recap` (0.1.1 — published + trusted 2026-08-15 via the first-release sequence; also bumped utils 0.3.1→0.3.2 and monitor 0.1.1→1.0.0 in the same Version Packages PR). `@fradser/pi-vision` is whitelisted but still 404 on the registry — its first version needs the manual publish + `npm trust` sequence above (changeset `vision-bridge.md` pending); until then every workflow run fails at its E404 (expected, documented). Never published and still in the monorepo: code-context, mattpocock, agent-teams (`@fradser/pi-agent-teams`); the former lark and teammate packages were removed from the repo. Single-package repo `GitAgentHQ/pi-git-agent` (unscoped `pi-git-agent`) has its own simpler publish workflow.
