...
2. The leader tool surface is `teammate_spawn`, `teammate_shutdown`, `send_message`, `task_create`, and `task_list`. Worker capabilities add `task_claim` and `task_submit`; `send_message` is the unified reporting and peer-message primitive. Its synchronous routing outcomes are `steered` only for an accepted active control-stream write and `queued` while the harness still owns delivery; inbox/outbox writes are queued, and neither outcome claims recipient processing. Teammate stall health is a separate `[agent] event`, never a `[message]` suffix.
...
5. Task completion is gated by the effective task verify prompt, falling back to the agent role's verify prompt; a fresh one-shot reviewer answers VERDICT: PASS/FAIL. Confirmed shutdown or crashes release claimed tasks; an unconfirmed close retains ownership (see [[project_teammate_shutdown_delivery]]), while the board persists for later inspection and runtime rosters do not.
...
10. Definition resolution: filesystem scopes (user < project < project-local) always outrank generated session roles, which only fill names no file defines; a re-spawn with an explicit inline definition replaces a stale session role of the same name but never touches files. Each spawn incarnation gets one assignment-finish entry, not a process-exit announcement; a later shutdown of that incarnation renders no second event row.
11. A recorded terminal report is readable without a resend: `send_message` against a terminally-reported teammate returns a truthful `not-sent` pi-kit lifecycle result with the recorded report and duplicate-resend warning, and a `reopen=true` result carries `PRIOR TERMINAL REPORT`. Never steer a teammate to repeat an already-sent report — the repeat arrives as a second identical leader turn; if delivery has not arrived yet, end the turn and wait. `task_list` is a board snapshot, not a completion wait mechanism.
