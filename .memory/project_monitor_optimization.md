...
- Do not intercept native `bash` calls with a package-level `tool_call` guardrail. `monitor_start` is an opt-in capability selected by concise system guidance for noisy, long-running, or asynchronous work; native bash remains responsible for synchronous execution. Do not add an `allow-sync` suffix or command-pattern classifier to override bash behavior.
- Model-facing terminal content should be compact structured text rather than pretty-printed JSON: retain JSON for arbitrary nested result payloads and machine metadata, but omit undefined fields and do not repeat captures or parsed results.
- Do not expose a model-facing output reader or status poller. Include a bounded source-labelled diagnostic tail in the one terminal result; the `/monitor` TUI may display fuller retained output without putting it into LLM context. Do not send progress batches into model context by default.
- User-facing monitor reports must not expose internal monitor identifiers such as `monitor_1`. Keep ids internal for manager operations, but omit them from startup text, terminal report content, transcript labels, and non-TUI notifications; show only the monitor description and result fields.
