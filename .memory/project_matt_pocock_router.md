...
- `src/index.ts` owns the single route menu, explicit commands (`status`, `transition`, `end`), the `matt_pocock_workflow` tool, procedure injection, UI status, session restoration, and concise active-phase or available-workflow `before_agent_start` guidance.
